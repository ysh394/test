	...
	<div id="webtoonWrap">
		...
		<main id="webtoonMain" class="DayMon">
			<div id="webtoonMainTop">
				...
				<c:forEach var="webtoonMonRecom" items="${mtdWebtoonMonRecom}">
					<article>
					<a href="/webtoonList?title=${webtoonMonRecom.title }">
						<div class="imgArea imgOver">
							<img src="/mainImg/${webtoonMonRecom.title}/${webtoonMonRecom.systemFile}" alt="이미지">
							<input type="hidden" value="${webtoonMonRecom.title}" class="hide">
						</div>
						<h3>${webtoonMonRecom.title}</h3>
						<h4>${webtoonMonRecom.writer}</h4>
						<span>${webtoonMonRecom.content}</span>
					</a>
					</article>
				</c:forEach>
