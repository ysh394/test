	...
	<div id="webtoonWrap">
		...
		<main id="webtoonMain" class="DayMon">
			<div id="webtoonMainTop">
				...
				<c:forEach var="webtoonSatRecom" items="${mtdWebtoonSatRecom}">
					<article>
					<a href="/webtoonList?title=${webtoonSatRecom.title }">
						<div class="imgArea imgOver">
							<img src="/mainImg/${webtoonSatRecom.title}/${webtoonSatRecom.systemFile}" alt="이미지">
							<input type="hidden" value="${webtoonSatRecom.title}" class="hide">
						</div>
						<h3>${webtoonSatRecom.title}</h3>
						<h4>${webtoonSatRecom.writer}</h4>
						<span>${webtoonSatRecom.content}</span>
						</a>
					</article>
				</c:forEach>
