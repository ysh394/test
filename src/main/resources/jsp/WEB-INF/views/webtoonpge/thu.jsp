	...
	<div id="webtoonWrap">
		...
		<main id="webtoonMain" class="DayMon">
			<div id="webtoonMainTop">
				...
				<c:forEach var="webtoonThuRecom" items="${mtdWebtoonThuRecom}">
					<article>
					<a href="/webtoonList?title=${webtoonThuRecom.title }">
						<div class="imgArea imgOver">
							<img src="/mainImg/${webtoonThuRecom.title}/${webtoonThuRecom.systemFile}" alt="이미지">
							<input type="hidden" value="${webtoonThuRecom.title}" class="hide">
						</div>
						<h3>${webtoonThuRecom.title}</h3>
						<h4>${webtoonThuRecom.writer}</h4>
						<span>${webtoonThuRecom.content}</span>
						</a>
					</article>
				</c:forEach>
				</section>
